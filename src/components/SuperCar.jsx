        ...
        <div className="snap-start flex-shrink-0 w-[340px] mx-2">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition-all duration-500">
                <div className="relative group">
                    ...
                    <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex space-x-1">
                        {car.images.map((_, idx) => (
                            <div
                                key={idx}
                                className={`w-2 h-2 rounded-full transition-all ${(currentImageIndex || 0) === idx ? 'bg-white' : 'bg-white/50'
                                    }`}
                            />
                        ))}
                    </div>
                ...
                <div className="p-5">
                    ...
                    <Link href={`/details/${car.id}`} className="no-underline hover:underline">
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex gap-2">
                            <button
                                onClick={() => window.location.href = `tel:${car.agency.contactPhone}`}
                            ...
                            <button
                                onClick={() => window.location.href = `https://wa.me/${car.agency.contactPhone}`}
...
export default function SportCarSection() {
    ...
    const fetchSportCars = useCallback(async () => {
        ...
        try {
            const categorySlug = 'sports-car-rental-dubai';
            ...
            if (!response.ok) {
                throw new Error(data.message || 'Failed to fetch sports cars');
            }
            ...
            if (err.name !== 'AbortError') {
                setError(err.message || 'An error occurred while fetching sports cars');
    ...
    }, [fetchSportCars, checkAuth]);

    // Original image change handler (keeping your exact logic)
    ...
    // Optimized wishlist toggle with toaster notifications
    ...
    const handleViewAll = useCallback(() => {
        router.push('/rent?category=Sports%20Car%20Rental%20Dubai');
    }, [router]);
        ...
        return (
            <section className="py-8 bg-gray-50">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="text-center text-gray-600">Loading sports cars...</div>
                </div>
            </section>
        );
    }
    ...
    return (
        <section className="py-8 bg-gray-50">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between mb-10">
                    <div>
                        ...
                        <p className="mt-2 text-[16px] text-gray-600">
                            Experience the thrill of our handpicked selection of high-performance sports cars, ready for your next adventure.
                        </p>
                    ...
                    <button
                        ...
                        className="mt-6 sm:mt-0 inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-blue-600 border border-blue-600 rounded-full hover:bg-blue-50 hover:text-blue-700 transition-all duration-200"
                    >
                        View all
                ...
                {/* Empty State */}
                {!error && cars.length === 0 && (
                    <div className="text-center text-gray-600">No sports cars found.</div>
                )}
